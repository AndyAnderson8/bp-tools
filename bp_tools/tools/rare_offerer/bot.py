...
class OffererBot(BotBase[OffererConfig]):
    ...
    poll_interval = 1.0  # fast poll; rate limit handled internally

    # 4 offers per minute = 15 seconds between actual POST calls
    _OFFER_COOLDOWN = 15.0
    ...
    def __init__(self, ctx: Any, tool_config: dict[str, Any], **kwargs: Any) -> None:
        ...
        self._existing: dict[int, int] = get_existing_offers(config_dir)
        self._log(f"Loaded {len(self._rare_ids)} rare items from cache.", True)
    ...
    def update(self) -> None:
        ...
        self._log(
            f"Checking to place offers... (cycle {self._cycle_count}) | "
        ...
        now = time.monotonic()
        if now - self._last_offer_time < self._OFFER_COOLDOWN:
            return  # don't advance — retry this item next cycle
